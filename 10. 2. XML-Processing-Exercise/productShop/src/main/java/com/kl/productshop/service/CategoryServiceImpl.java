...
import com.kl.productshop.domain.dtos.seed_dtos.CategoryImportDto;
...
//    }
    ...
    @Override
    public String seedCategoriesXml(CategoryImportRootDto categoryImportRootDto) {


        for (CategoryImportDto categorySeedDto : categoryImportRootDto.getCategoryImportDtoList()) {
            if(!this.validatorUtil.isValid(categorySeedDto)){
                this.validatorUtil.violations(categorySeedDto)
                        .forEach(violation -> System.out.println(violation.getMessage()));
